...
def cutblur(im1, im2, prob=1.0, alpha=1.0):
    ...
    if alpha <= 0 or np.random.rand(1) >= prob:  # np.random.rand(1)返回[0~1)的随机数
        return im1, im2

    cut_ratio = np.random.randn() * 0.01 + alpha  # np.random.randn()返回主要在-1.96~+1.96之间的随机数(p = 0.95)
...
def flip(im1, im2, prob=1.0):
    ...
    if hflip:
        im1 = torch.flip(im1, [3])
        im2 = torch.flip(im2, [3])
    if vflip:
        im1 = torch.flip(im1, [2])
        im2 = torch.flip(im2, [2])
    if rot90:
        im1 = torch.rot90(im1, 1, [3, 2])
        im2 = torch.rot90(im2, 1, [3, 2])
    return im1, im2
...
def rand_bbox(size, lam):
    W = size[2]
    H = size[3]
    cut_rat = np.sqrt(1. - lam)  # 平方
    cut_w = np.int(W * cut_rat)
    cut_h = np.int(H * cut_rat)

    # uniform
    cx = np.random.randint(W)  # [0, W)的整数
    cy = np.random.randint(H)  # [0, H)的整数

    bbx1 = np.clip(cx - cut_w // 2, 0, W)  # bbx1 (0, W)
