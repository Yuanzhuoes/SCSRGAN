...
import math
import numpy as np
from metrics.psnr_ssim import to_y_channel
...
def calculate_niqe(img, crop_border=4, input_order='HWC', convert_to='y'):
    """Calculate NIQE (Natural Image Quality Evaluator) metric.
    ...
    if input_order != 'HW':
        ...
        if convert_to == 'y':
            ...
            sr = cv2.cvtColor(sr / 255., cv2.COLOR_BGR2GRAY) * 255.
        sr = np.squeeze(sr)

    if crop_border != 0:
        sr = sr[crop_border:-crop_border, crop_border:-crop_border]

    niqe_result = niqe(sr, mu_pris_param, cov_pris_param, gaussian_window)
